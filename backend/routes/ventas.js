  ...
  try {
    ...
    if (!totalCalculado || totalCalculado <= 0) {
      totalCalculado = items.reduce((sum, item) => {
        ...
        return sum + ((precioBase + precioExtras) * cantidad);
      }, 0);
    ...
    const [ventaResult] = await connection.query(
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [folio, clienteId, totalCalculado, totalCalculado, metodo_pago, empleado || 'Cajero', notas || null]
    );
      ...
      if (item.extras && item.extras.length > 0) {
        for (const extra of item.extras) {
          await connection.query(
             ...
             VALUES (?, ?, ?, ?)`,
              [detalleId, extra.id, extra.nombre, parseFloat(extra.precio || 0)]
          );
        }
      }
    }
    ...
    if (clienteId) {
      const puntos = Math.floor(totalCalculado / 10); // 1 punto por cada $10
      ...
      await connection.query(
         ...
         SET total_compras = total_compras + ?,
             ...
             ultima_compra = NOW()
         WHERE id = ?`,
          [totalCalculado, puntos, clienteId]
      );
    }
